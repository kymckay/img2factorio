...
def main():
    ...
    for image in args.image:
        if not os.path.isfile(image):
            print("File doesn't exist: {}".format(image))
        else:
            # Open the image file
            try:
                img = Image.open(image)
            except IOError:
                print("Failed to open: {}".format(image))
                continue
            ...
            if args.preview:
                ...
                img.convert("1", dither=Image.NONE).save(os.path.join("preview",image))
                print("Saved Preview")
